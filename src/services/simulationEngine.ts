...
// Helper to calculate Chebyshev distance on a 2D grid
const getDistance = (x1: number, y1: number, x2: number, y2: number) => {
  return Math.max(Math.abs(x1 - x2), Math.abs(y1 - y2));
};

/**
 * Initializes a new simulated battle arena state
 */
export function initializeLocalSimulation(
  maxRound: number = 50,
  kingdomsConfig: Array<{ name: string; model: string }>
): BattleState {
...
/**
 * Advances the simulation by one round and calculates next state.
 */
export function processNextSimulationRound(battleState: BattleState): SimulationRoundResult {
